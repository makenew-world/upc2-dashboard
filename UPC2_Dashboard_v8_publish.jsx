...
function calcQuarterScheme(area,rawData,quarterScheme){const defs=quarterScheme[area]||[];const areaData=rawData.filter(r=>r.a===area);const total=areaData.reduce((s,r)=>s+r.v,0);return defs.map(s=>{let marAct=0;if(s.brands){areaData.forEach(r=>{if(s.brands.includes(r.b))marAct+=r.v;});}else marAct=total;const totalAct=s.janFebAct+marAct;return{...s,act:totalAct,marAct,pct:s.tgt>0?(totalAct/s.tgt)*100:0};});}
...
function Dash({area,onLogout,rawData,dataDate,mtdLabel,tgt,schemeDef,quarterScheme,quarterLabel,quarterPeriod}){
  ...
  return(
    <div style={{minHeight:"100vh",background:"#f4f6f8",fontFamily:"'Sarabun',system-ui,sans-serif",fontVariantNumeric:"tabular-nums",padding:"12px 12px 32px",maxWidth:480,margin:"0 auto"}}>
      <link href="https://fonts.googleapis.com/css2?family=Sarabun:wght@300;400;500;600;700;800&display=swap" rel="stylesheet"/>
      ...
      <div style={card}><div style={sec}>แยกตามแบรนด์</div>
        {brandSum.map((b,i)=>{const tgt=targets[b.b]||0;const pct=tgt>0?(b.v/tgt)*100:0;const col=pctCol(pct);return(<div key={i} style={{padding:"8px 0",borderBottom:i<brandSum.length-1?"1px solid #f8fafc":"none"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:3}}><div style={{display:"flex",alignItems:"center",gap:6}}><span style={{width:8,height:8,borderRadius:4,background:BC[b.b]||"#94a3b8",flexShrink:0}}/><span style={{fontSize:13,fontWeight:700,color:"#1e293b"}}>{b.b}</span></div><span style={{fontSize:13,fontWeight:800,color:tgt>0?col:"#64748b"}}>{tgt>0?`${pct.toFixed(0)}%`:"—"}</span></div>{tgt>0&&<Bar pct={pct} color={col}/>}<div style={{display:"flex",justifyContent:"space-between",fontSize:10,color:"#94a3b8",marginTop:2}}><span>{fmt(b.v)}</span>{tgt>0&&<span>เป้า {fmt(tgt)}</span>}</div></div>);})}</div>

      {schemes.length>0&&(<div style={card}><div style={sec}>🎯 Incentive Scheme (เป้า {curMonth})</div>
        <div style={{display:"grid",gridTemplateColumns:schemes.length<=2?"1fr 1fr":`repeat(${Math.min(schemes.length,3)},1fr)`,gap:8}}>
          {schemes.map((s,i)=>(<div key={i} style={{background:"#f8fafc",borderRadius:12,padding:10,textAlign:"center"}}><div style={{fontSize:10,fontWeight:700,color:"#64748b",marginBottom:2,lineHeight:1.2}}>{s.name}</div><Mini pct={s.pct} size={78}/><div style={{fontSize:11,fontWeight:700,color:"#1e293b"}}>{fmt(s.act)}</div><div style={{fontSize:9,color:"#94a3b8"}}>เป้า {fmt(s.tgt)}</div></div>))}</div></div>)}
      ...
      {isMgr&&areaBreak&&(<div style={card}><div style={sec}>📍 แยกตาม Area — กดเพื่อดู Scheme</div>
        {areaBreak.map((a,i)=>{const col=pctCol(a.pct);const isOpen=expandArea===a.area;const areaSchemes=isOpen?calcScheme(a.area,rawData||[],schemeDef||{}):[];const areaQ1=isOpen?calcQuarterScheme(a.area,rawData||[],quarterScheme||{}):[];
          return(<div key={i}><div onClick={()=>setExpandArea(isOpen?null:a.area)} style={{padding:"10px 0",borderBottom:!isOpen&&i<4?"1px solid #f8fafc":"none",cursor:"pointer"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}><div style={{display:"flex",alignItems:"center",gap:6}}><span style={{fontSize:12,transition:"transform .2s",transform:isOpen?"rotate(90deg)":"rotate(0)",display:"inline-block"}}>▸</span><span style={{fontSize:14,fontWeight:700,color:"#1e293b"}}>{a.area}</span></div><div style={{textAlign:"right"}}><span style={{fontSize:14,fontWeight:800,color:col}}>{a.pct.toFixed(1)}%</span><span style={{fontSize:10,color:"#94a3b8",marginLeft:6}}>{fmt(a.act)}</span></div></div><Bar pct={a.pct} color={col} h={7}/><div style={{display:"flex",justifyContent:"space-between",fontSize:10,color:"#94a3b8",marginTop:3}}><span>Actual {fmt(a.act)}</span><span>Target {fmt(a.tgt)}</span></div></div>
            {isOpen&&(<div style={{background:"#f8fafc",borderRadius:12,padding:12,margin:"4px 0 4px",borderLeft:"3px solid "+col}}><div style={{fontSize:11,fontWeight:700,color:"#64748b",marginBottom:8}}>{curMonth} Scheme — {a.area}</div>{areaSchemes.map((s,j)=>{const sc=pctCol(s.pct);return(<div key={j} style={{display:"flex",alignItems:"center",gap:10,padding:"6px 0",borderBottom:j<areaSchemes.length-1?"1px solid #e9ecf0":"none"}}><div style={{flex:1,minWidth:0}}><div style={{fontSize:12,fontWeight:600,color:"#374151"}}>{s.name}</div><div style={{fontSize:10,color:"#94a3b8"}}>{fmt(s.act)} / {fmt(s.tgt)}</div></div><div style={{width:50,textAlign:"right",fontSize:14,fontWeight:800,color:sc}}>{s.pct.toFixed(0)}%</div></div>);})}</div>)}
            {isOpen&&areaQ1.length>0&&(<div style={{background:"#faf5ff",borderRadius:12,padding:12,margin:"4px 0 12px",borderLeft:"3px solid #8b5cf6"}}><div style={{fontSize:11,fontWeight:700,color:"#7c3aed",marginBottom:8}}>{quarterLabel} Scheme — {a.area}</div>{areaQ1.map((s,j)=>{const sc=pctCol(s.pct);return(<div key={j} style={{display:"flex",alignItems:"center",gap:10,padding:"6px 0",borderBottom:j<areaQ1.length-1?"1px solid #ede9fe":"none"}}><div style={{flex:1,minWidth:0}}><div style={{fontSize:12,fontWeight:600,color:"#374151"}}>{s.name}</div><div style={{fontSize:10,color:"#94a3b8"}}>{fmt(s.act)} / {fmt(s.tgt)}</div></div><div style={{width:50,textAlign:"right",fontSize:14,fontWeight:800,color:sc}}>{s.pct.toFixed(0)}%</div></div>);})}</div>)}
          </div>);})}</div>)}
