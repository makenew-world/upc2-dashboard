...
function Dash({area,onLogout,rawData,dataDate,mtdLabel,tgt,schemeDef,quarterScheme,quarterLabel,quarterPeriod}){
  ...
  const q1Schemes=useMemo(()=>{const defs=(quarterScheme||{})[isMgr?"MGR":area]||[];return defs.map(s=>{let marAct=0;if(s.brands){data.forEach(r=>{if(s.brands.includes(r.b))marAct+=r.v;});}else marAct=totalMTD;const totalAct=s.janFebAct+marAct;return{...s,act:totalAct,marAct,pct:s.tgt>0?(totalAct/s.tgt)*100:0};});},[area,data,totalMTD,isMgr]);
    ...
    <div style={{minHeight:"100vh",background:"#f4f6f8",fontFamily:"'Sarabun',system-ui,sans-serif",padding:"12px 12px 32px",maxWidth:480,margin:"0 auto"}}>
      ...
      {q1Schemes.length>0&&(<div style={card}><div style={sec}>📊 {quarterLabel} Incentive Scheme ({quarterPeriod})</div>
        <div style={{display:"grid",gridTemplateColumns:q1Schemes.length<=2?"1fr 1fr":`repeat(${Math.min(q1Schemes.length,3)},1fr)`,gap:8}}>
          {q1Schemes.map((s,i)=>(<div key={i} style={{background:"#faf5ff",borderRadius:12,padding:10,textAlign:"center",border:"1px solid #ede9fe"}}><div style={{fontSize:10,fontWeight:700,color:"#7c3aed",marginBottom:2,lineHeight:1.2}}>{s.name}</div><Mini pct={s.pct} size={78}/><div style={{fontSize:11,fontWeight:700,color:"#1e293b"}}>฿{fmtK(s.act)}</div><div style={{fontSize:9,color:"#94a3b8"}}>เป้า {quarterLabel} ฿{fmtK(s.tgt)}</div><div style={{fontSize:8,color:"#a78bfa",marginTop:2}}>{curMonth} ฿{fmtK(s.marAct)}</div></div>))}</div></div>)}
